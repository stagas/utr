...
const series = (context, series2, stack, env, callback) => {
  let length = series2.length
  for (let index = length - 1; index >= 0; index--) {
    let value = series2[index]
...
function printLegacyPlugin(plugin, value, env, refs, depth) {
  ...
  function print(value2) {
    return printStack(value2, depth + 1, refs, env)
  }
  function indent(str) {
...
function printValue(value, stack, env, refs, depth) {
  ...
  if (value === TRUE_VAL)
    return TRUE_PRINTED
  if (value === FALSE_VAL)
    return FALSE_PRINTED
  if (value === NULL_VAL)
    return NULL_PRINTED
  let type = typeof value
  if (type === UNDEFINED_TYPE)
    return UNDEFINED_PRINTED
  if (type === STRING_TYPE)
  ...
  if (type === FUNCTION_TYPE)
    return printFunction(value, env.opts.printFunctionName)
  let str = toString.call(value)
  if (str === WEAKMAP_STR)
    return WEAKMAP_PRINTED
  if (str === WEAKSET_STR)
    return WEAKSET_PRINTED
  if (str === FUNCTION_STR || str === GENERATOR_STR)
    return printFunction(value, env.opts.printFunctionName)
  if (str === SYMBOL_STR)
    return printSymbol(value)
  if (str === DATE_STR)
  ...
  if (str === REGEXP_STR)
    return printRegExp(value, env.opts.escapeRegex)
  if (refs.has(value)) {
    return CIRCULAR_STR
  } else {
    refs.add(value)
  ...
  if (hitMaxDepth) {
    if (matchedArrayLike)
      return OPEN_BRACKET + matchedArrayLike + CLOSE_BRACKET
    if (str === MAP_STR)
      return MAP_MAX_PRINTED
    if (str === SET_STR)
      return SET_MAX_PRINTED
...
function printStack(value, depth, refs, env) {
  let result = ''
  let stack = new Stack()
  stack.push(value)
  while (stack.length) {
...
function validateOptions(opts) {
  ...
  if (opts.min && opts.indent !== void 0 && opts.indent !== 0) {
...
var prettyFormat2 = prettyFormat

export { prettyFormat2 as default }
